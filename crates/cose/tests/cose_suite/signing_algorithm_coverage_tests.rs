...
use crate::support::{
    gen_ed25519, gen_mldsa44, gen_mldsa65, gen_mldsa87, gen_p256, gen_p384, gen_p521,
    gen_secp256k1, sample_payload, TestKey,
};
...
fn all_supported_signing_algorithms_roundtrip_attached_and_detached() {
    ...
    let cases: [SigningCase; 8] = [
        ("Ed25519", gen_ed25519, 64),
        ("ES256", gen_p256, 64),
        ("ES384", gen_p384, 96),
        ("ES512", gen_p521, 132),
        ("ES256K", gen_secp256k1, 64),
        ("ML-DSA-44", gen_mldsa44, 2_420),
        ("ML-DSA-65", gen_mldsa65, 3_309),
        ("ML-DSA-87", gen_mldsa87, 4_627),
    ];
    ...
    for (label, make_key, expected_signature_len) in cases {
        ...
        let attached_payload = cose_verify1(&attached, |_, requested_kid| {
        ...
        .unwrap();
        assert_eq!(
            attached_payload.as_slice(),
            payload.as_slice(),
            "attached COSE_Sign1 {label}"
        );

        let detached = cose_sign1_detached(key.alg, &payload, &key.private, Some(kid)).unwrap();
        cose_verify1_detached(&detached, &payload, |_, requested_kid| {
            if requested_kid == kid {
                Some(key.public.clone())
            } else {
                None
