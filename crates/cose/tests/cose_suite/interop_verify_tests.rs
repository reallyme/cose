...
use crate::support::{gen_ed25519, gen_p256, gen_secp256k1, sample_payload, test_kid};
...
fn build_reordered_header_sign1(key: &crate::support::TestKey, payload: &[u8]) -> Vec<u8> {
    let protected_map = Value::Map(vec![
        ...
        (Value::Integer(1.into()), Value::Integer((-19).into())),
    ]);
...
fn ecdsa_der_encoded_signature_is_rejected() {
    ...
    let err = cose_verify1(&mutated, |_, kid| {
        (kid == test_kid()).then(|| key.public.clone())
    })
...
#[test]
fn resolver_binds_same_width_ec_keys_to_the_expected_algorithm() {
    let payload = sample_payload();
    let p256 = gen_p256();
    let secp256k1 = gen_secp256k1();

    for (signed, wrong_key) in [
        (
            cose_sign1(p256.alg, &payload, &p256.private, Some(test_kid()))
                .expect("P-256 fixture must sign"),
            &secp256k1,
        ),
        (
            cose_sign1(
            ...
            .expect("secp256k1 fixture must sign"),
            &p256,
        ),
    ] {
        let error = cose_verify1(&signed, |expected_algorithm, kid| {
            (expected_algorithm == wrong_key.alg && kid == test_kid())
                .then(|| wrong_key.public.clone())
        })
        .expect_err("resolver must not return a key bound to another algorithm");
        assert_eq!(error, CoseError::KeyNotResolved);
    }
}
