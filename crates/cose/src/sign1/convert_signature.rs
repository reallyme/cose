...
//! Ed25519 and ML-DSA are already encoded in their COSE wire forms, but their
//! exact widths are enforced here so a permissive backend cannot accept a
...
/// Exact width for algorithms whose backend already uses the COSE encoding.
...
fn validate_direct_signature_len(alg: Algorithm, signature: &[u8]) -> Result<(), CoseError> {
    if direct_signature_len(alg).is_some_and(|expected| signature.len() != expected) {
...
fn der_to_fixed_width(der: &[u8], coordinate_len: usize) -> Result<Vec<u8>, CoseError> {
    let sequence_tag = *der.first().ok_or(CoseError::InvalidSignatureEncoding)?;
...
#[cfg(test)]
#[path = "convert_signature_tests.rs"]
mod tests;
