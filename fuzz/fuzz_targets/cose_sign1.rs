...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = reallyme_cose::cose_verify1(data, |_, _| None);
    let _ = reallyme_cose::cose_verify1(data, |_, kid| Some(kid.to_vec()));
    ...
    if let Some((split_byte, rest)) = data.split_first() {
        ...
        let _ = reallyme_cose::cose_verify1_detached(cose_bytes, payload, |_, _| None);
        let _ =
            reallyme_cose::cose_verify1_detached(cose_bytes, payload, |_, kid| Some(kid.to_vec()));
    }
});
